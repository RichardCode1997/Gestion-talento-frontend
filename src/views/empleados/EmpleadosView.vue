...
// ── Filtro ──
// NOTA: el SUPERADMIN ya viene filtrado desde el backend (EmpleadosService).
// Aquí solo aplicamos filtros de estado y búsqueda local.
...
// ── Lógica de permisos ──
// Ahora usan emp.correoUsuario y emp.rolUsuario del EmpleadoResponseDTO.
// Si el empleado no tiene usuario vinculado, correoUsuario y rolUsuario son null.

function puedeCambiarEstado(emp) {
  // No puedes cambiar tu propio estado
  if (emp.correoUsuario === correoUsuario) return false
  // ADMINISTRADOR solo puede ser gestionado por SUPERADMIN
  if (emp.rolUsuario === 'ADMINISTRADOR' && rolUsuario !== 'SUPERADMIN') return false
  // Solo SUPERADMIN y ADMINISTRADOR pueden cambiar estados
...
// ── Navegación ──
function irANuevo()   { router.push('/empleados/nuevo') }
function irAEditar(id) { router.push(`/empleados/${id}/editar`) }
...
async function cambiarEstado(emp, nuevoEstado) {
  if (emp.correoUsuario === correoUsuario) {
    mostrarToast('No puedes cambiar tu propio estado.', 'error')
    return
  }
  try {
    await empleadosService.cambiarEstado(emp.idEmpleado, nuevoEstado)
    await cargarEmpleados()
        ...
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr v-for="emp in empleadosFiltrados" :key="emp.idEmpleado">
              ...
              <!-- correoUsuario y rolUsuario vienen del EmpleadoResponseDTO -->
              <td class="td-correo">{{ emp.correoUsuario ?? '—' }}</td>
              <td>
                <span :class="['badge', getRolClass(emp.rolUsuario)]">
                  {{ emp.rolUsuario ?? '—' }}
                </span>
              </td>

              <td>
                <!-- Dropdown estado: visible solo si tiene permiso -->
                ...
                <!-- Sin permiso: solo badge sin dropdown -->
              ...
              <td>
                <div class="actions-cell">
                  ...
                  <button class="action-btn delete"
                          @click="puedeEliminar(emp) && confirmarEliminar(emp)"
                          :disabled="!puedeEliminar(emp)"
                          :style="!puedeEliminar(emp) ? 'opacity:0.4; cursor:not-allowed' : ''"
                          title="Eliminar">
...
.state-box { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; background: #fff; border-radius: var(--radius); padding: 60px 20px; text-align: center; color: var(--text-muted); font-size: 14px; box-shadow: var(--card-shadow); }
...
.table-card { background: #fff; border-radius: var(--radius); box-shadow: var(--card-shadow); overflow: visible; }
...
.emp-avatar { width: 36px; height: 36px; border-radius: 50%; flex-shrink: 0; background: linear-gradient(135deg, #4f8cff, #38d9a9); color: #fff; font-weight: 700; font-size: 12px; display: flex; align-items: center; justify-content: center; }
...
.badge-sistemas   { background: rgba(99,110,250,0.12); color: #636efa; }
...
.badge-estado { display: inline-flex; align-items: center; gap: 5px; padding: 5px 10px; border-radius: 999px; font-size: 12px; font-weight: 600; cursor: pointer; border: none; }
...
.estado-dropdown { position: absolute; top: calc(100% + 6px); left: 0; background: #fff; border: 1px solid #e5e9f0; border-radius: 10px; min-width: 130px; box-shadow: 0 4px 16px rgba(0,0,0,0.1); z-index: 99; overflow: hidden; }
.dd-item { display: flex; align-items: center; gap: 8px; padding: 9px 14px; cursor: pointer; font-size: 13px; }
